  ...
  if (isTensor) {
    ...
    if (resultScalar) {
      ...
      if (cppOp == "ceildiv") {
        ...
        emit("    " + var + "[_i] = " + buildMinMaxFmod(lhsE, rhsE) + ";");
      } else {
...
void CUDACodeGen::emitMathOp(Operation *op) {
  ...
  if (isFma) {
    ...
    if (isTensor) {
      ...
      emit("for (int _i = 0; _i < " + llvm::Twine(nElems).str() + "; _i++)");
      std::string a = getElemExpr(op->getOperand(0), "_i");
      std::string b = getElemExpr(op->getOperand(1), "_i");
      std::string c = getElemExpr(op->getOperand(2), "_i");
      emit("    " + var + "[_i] = " + fmaFn + "(" + a + ", " + b + ", " + c +
           ");");
    } else {
      std::string a = getVar(op->getOperand(0));
  ...
  if (isTensor) {
    ...
    emit("#pragma unroll");
    emit("for (int _i = 0; _i < " + llvm::Twine(nElems).str() + "; _i++)");
    std::string elemE = getElemExpr(op->getOperand(0), "_i");
    if (isAbs)
      emit("    " + var + "[_i] = " + absExpr(elemE) + ";");
    else
      emit("    " + var + "[_i] = " + fn + "(" + elemE + ");");
  } else {
    if (isAbs)
...
void CUDACodeGen::emitWarpSpecialize(gpu::WarpSpecializeOp op) {
  ...
  auto caps = op.getExplicitCaptures();
  ...
  if (auto nvmmaShared =
    ...
    for (int dim = 0; dim < smemRank; ++dim)
      msgToOffset *= LinearLayout::strided1D(shapePerCTA[dim] / blockShape[dim],
                                             blockShape[dim], kMsg,
                                             outDimNames[dim]);
    auto ctaLayout = ttg::getCTALayout(smemTy.getEncoding());
